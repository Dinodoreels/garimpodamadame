...
export default function Lote() {
  ...
  const { data: cmsPage } = useCMSPageBySlugOrHome('lote');
  const { cep: shippingCep } = useCatalogShippingCep();

  const products = useMemo(
    () => (allProducts || []).filter((p) => (p as any).is_lote === true),
    [allProducts]
  );
        ...
        {cmsHeaderSections.length > 0 ? (
          ...
          <section className="bg-secondary/50 py-8 lg:py-12">
            <div className="container">
              <h1 className="font-display text-3xl lg:text-4xl font-light tracking-wide">Venda em Lote</h1>
              <p className="text-muted-foreground mt-2 text-sm">
                Compre em maior quantidade com condições especiais
              </p>
            </div>
          </section>
        )}
        ...
        <section className="py-6 border-b">
          <div className="container space-y-4">
            ...
            <div className="flex items-center gap-2">
              <Switch
                id="only-available-lote"
                checked={filters.onlyAvailable}
                onCheckedChange={(v) => setFilters({ ...filters, onlyAvailable: v })}
              />
              <Label htmlFor="only-available-lote" className="text-sm cursor-pointer">
                Somente disponíveis
              </Label>
            </div>
