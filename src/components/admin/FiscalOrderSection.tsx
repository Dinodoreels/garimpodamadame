...
export function FiscalOrderSection({ orderId, orderStatus }: { orderId: string; orderStatus: string }) {
  ...
  const errors = Array.isArray(document?.validation_errors) ? document.validation_errors : [];
  return <div className="space-y-4">
    ...
    {errors.length > 0 && <Alert variant="destructive"><AlertTitle>Falta corrigir</AlertTitle><AlertDescription><ul className="list-disc pl-4">{errors.map((error: string) => <li key={error}>{error}</li>)}</ul></AlertDescription></Alert>}
