...
export function WhatsAppCampaignsTab() {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>Nova campanha</CardTitle><CardDescription>Monte a mensagem e confira a prévia antes de enviar ou agendar.</CardDescription></CardHeader><CardContent className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2"><Button variant="outline" disabled={saveCampaign.isPending} onClick={() => saveCampaign.mutate('preview')}><Eye className="mr-2 h-4 w-4" />Visualizar</Button><Button variant="secondary" disabled={saveCampaign.isPending} onClick={() => saveCampaign.mutate('draft')}>Salvar rascunho</Button><Button variant="outline" disabled={!campaignForm.scheduled_for || saveCampaign.isPending} onClick={() => saveCampaign.mutate('schedule')}><CalendarClock className="mr-2 h-4 w-4" />Agendar</Button><Button disabled={saveCampaign.isPending} onClick={() => saveCampaign.mutate('send')}><Send className="mr-2 h-4 w-4" />Enviar agora</Button></div>
