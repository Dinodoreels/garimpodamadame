...
function ConsentAudit() {
  ...
  useEffect(() => { supabase.from('user_legal_consents').select('id,user_id,document_key,document_version,source,accepted_at').order('accepted_at', { ascending: false }).limit(500).then(({ data }) => { setItems(data ?? []); setLoading(false); }); }, []);
  return <Card><CardHeader><CardTitle>Histórico de aceites</CardTitle></CardHeader><CardContent>{loading ? <Loader2 className="h-6 w-6 animate-spin" /> : <Table><TableHeader><TableRow><TableHead>Usuário</TableHead><TableHead>Documento</TableHead><TableHead>Versão</TableHead><TableHead>Origem</TableHead><TableHead>Data</TableHead></TableRow></TableHeader><TableBody>{items.map(item => <TableRow key={item.id}><TableCell className="font-mono text-xs">{item.user_id}</TableCell><TableCell>{item.document_key}</TableCell><TableCell>{item.document_version}</TableCell><TableCell>{item.source}</TableCell><TableCell>{new Date(item.accepted_at).toLocaleString('pt-BR')}</TableCell></TableRow>)}</TableBody></Table>}</CardContent></Card>;
}
