...
BEGIN
  ...
  IF v_variant_id IS NULL AND v_item.barcode IS NOT NULL THEN
    ...
    WHERE barcode = v_item.barcode OR gtin = v_item.barcode
  ...
  IF v_variant_id IS NULL THEN
    ...
    VALUES (v_item.title, p_description, v_handle, v_item.category, v_item.brand, v_price, 'active', true, 'used', now(), COALESCE(v_item.ai_data, '{}'::jsonb))
  ...
  ELSE
    ...
    SET inventory_quantity = inventory_quantity + v_item.quantity,
        ...
        barcode = COALESCE(barcode, v_item.barcode),
        gtin = COALESCE(gtin, v_item.barcode),
        is_available = true,
        updated_at = now()
    WHERE id = v_variant_id;
    UPDATE public.products
    SET title = COALESCE(NULLIF(title, ''), v_item.title),
        description = COALESCE(NULLIF(description, ''), p_description),
        product_type = COALESCE(product_type, v_item.category),
        vendor = COALESCE(vendor, v_item.brand),
        price = v_price,
        is_available = true,
        status = 'active',
        updated_at = now()
  ...
  SET product_id = v_product_id, variant_id = v_variant_id, sku = v_sku,
      approved_price = v_price, approved_by = p_actor_id, approved_at = now(),
  ...
  VALUES (p_item_id, v_variant_id, v_item.location_id, 'auto_published', v_item.quantity, v_item.state, 'AVAILABLE', 'Cadastro automático pelo Garimpo Scan', p_actor_id);
  ...
  VALUES (v_product_id, v_variant_id, 'product', jsonb_build_object('source','inbound_auto_publish','item_id',p_item_id));
  INSERT INTO public.bling_sync_queue(product_id, variant_id, action, payload)
  VALUES (v_product_id, v_variant_id, 'stock', jsonb_build_object('source','inbound_auto_publish','item_id',p_item_id));
